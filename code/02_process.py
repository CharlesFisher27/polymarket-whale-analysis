...
def _build_event_windows(comments_df: pd.DataFrame, prices_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Uses a ±7-day nearest-observed-price window for all markets. This is
    appropriate for long-horizon prediction markets (elections, FIFA, geopolitics)
    where meaningful price shifts occur over days, not hours.

    price_before = last observed price strictly before the comment (within 7 days)
    price_after  = first observed price at or after the comment (within 7 days)
    """
    if prices_df.empty:

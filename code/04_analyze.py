...
def apply_sparse_price_filter(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove markets with effectively constant price histories (std of |Δp| < 0.001).
    These markets have broken or missing price data and contribute no signal.
    Matches the filter in 05_generate_figures.py.
    """
    print("\nApplying sparse-price filter...")
    before_mkts = df["market_label"].nunique()
    before_rows = len(df)
    ...
    broken    = mkt_std[mkt_std < 0.001].index
    df        = df[~df["market_label"].isin(broken)].copy()

    after_mkts = df["market_label"].nunique()
    print(f"  Removed {len(broken)} markets with std(|Δp|) < 0.001 "
          f"({before_rows - len(df):,} rows dropped); {after_mkts} markets remain")
    return df


def apply_fed_filter(df: pd.DataFrame, raw_dir: Path) -> pd.DataFrame:
    """
    Remove Fed Policy markets (no meaningful whale activity).
    Identifies markets by keyword-matching question text in market.json,
    matching the filter in 05_generate_figures.py.
    """
    print("\nApplying Fed Policy market filter...")
    before_mkts = df["market_label"].nunique()
    before_rows = len(df)
...
def main():
    ...
    # 2. Filter — must match the three filters described in the paper's Data section
    df = apply_convergence_filter(df)
    df = apply_sparse_price_filter(df)
    df = apply_fed_filter(df, RAW_DIR)
